...
export class AppComponent {
  ...
  whoseTurnDiv: HTMLDivElement = document.getElementById("whoseTurnDiv") as HTMLDivElement;


  onFieldClicked = (x: number, y: number) => {
    if (!this.isGameFinished && this.fields[x][y] == this.FIELD_STATUSES.EMPTY) {
      this.crossField(x, y);
      if (this.checkWin()) {
        ...
        setTimeout(() => {
          this.computerMove();
          this.whoseTurnDiv.innerText = "ruch gracza";
        }, 800);
    }
  };
  ...
  ngOnInit() {
    ...
    this.whoseTurnDiv = document.getElementById("whoseTurnDiv") as HTMLDivElement;
  };
  ...
  checkWin() {
    ...
    if (this.fields[0][2] != this.FIELD_STATUSES.EMPTY
      && this.fields[1][1] == this.fields[0][2]
      && this.fields[2][0] == this.fields[0][2])
      return true;

    return false;
  }

  whoseTurn = "ruch gracza";
